...
## Locale Labels And Selector Options

If you provide multiple locales, you can also define labels and locale-code aliases for the footer locale selector:
...
new PlusAuthWidget(document.body, {
  locale: {
    ...
    locales: {
      ...
      tr: { label: 'Türkçe', codes: ['tr', 'tr-TR'] }
    }
  }
})
```

When `context.params.ui_locales` is present, the widget selects the first matching locale from that space-separated preference list.

```javascript
window.PlusAuth = {
  params: {
    ui_locales: 'tr-TR en-US'
  }
}
```
