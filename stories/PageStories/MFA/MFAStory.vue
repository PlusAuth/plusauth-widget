...
const injectionObject = computed(() =>{
  if (props.prompt && (promptInjectionMap as any)[props.mode]?.[props.prompt]) {
    return  (promptInjectionMap as any)[props.mode]?.[props.prompt]
  }
  return undefined
})
</script>
...
<template>
  <WidgetWrapper
    :context="{
      ...
      details: {
        challenges: ['sms', 'email', 'otp', 'sc', 'webauthn', 'fv'],
